...
const userPostsQuery = z.object({
	method: z.enum(["uid", "id", "un"]),
	id: z.string(),
	fromP: z.string().optional().default("1"),
	toP: z.string().optional().default("20"),
	needForumName: z.string().optional().default("true"),
});

const forumThreadsQuery = z.object({
	fname: z.string(),
	sort: z.string().optional().default("1"),
	count: z.string().optional().default("50"),
	depth: z.enum(["first", "all"]).optional().default("first"),
	withComments: z.string().optional().default("false"),
	maxPages: z.string().optional().default("5"),
});

const threadPostsQuery = z.object({
	tid: z.string(),
	withComments: z.string().optional().default("false"),
});
...
function toText(
	...
	return content
		.map((item) => {
			...
			if (item.type === 2 || item.type === 11) return item.c ? `#(${item.c})` : "";
...
function toThreadPost(post: {
	id: string;
	floor: number;
	time: number;
	authorId?: string;
	content: Array<{ type: number; text?: string; c?: string }>;
	agree?: { agreeNum?: string | number; agree_num?: string | number } | undefined;
	author?: unknown;
	subPostList?: {
		subPostList: Array<{
			id: string;
...
export const exportRoute = new Hono()
	.get("/userPosts", zValidator("query", userPostsQuery), async (c) => {
	...
	.get("/forumThreads", zValidator("query", forumThreadsQuery), async (c) => {
		const { fname, sort, count, depth, withComments, maxPages } = c.req.valid("query");
		const includeComments = withComments === "true";
		const threadCount = Math.min(Math.max(Number(count) || 50, 1), 300);
		const maxThreadPostPages = Math.min(Math.max(Number(maxPages) || 5, 1), 20);
		...
		return streamSSE(c, async (stream) => {
			try {
				...
				threads = threads.filter((thread) => !thread.isTop).slice(0, threadCount);
					...
					for (const thread of threads) {
						try {
							...
							threadData.push({
								tid: thread.id,
								title: thread.title,
								authorName: readAuthorName(thread.author) || thread.authorId || "",
	...
	.get("/threadPosts", zValidator("query", threadPostsQuery), async (c) => {
