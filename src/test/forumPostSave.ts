...
async function processPostList(pidList: string[]) {
  ...
  for (let i = 0; i < pidList.length; i += chunkSize) {
    ...
    await Promise.all(promises); // 并发处理帖子
    if (i % 100 === 0) {
      logger.info(`已处理${i}个主题帖，已耗时${(Date.now() - startTime) / 1000}秒`);
    }
    await sleep(delay * Math.random()); // 添加延迟
  }
  const endTime = Date.now();
