...
async function processPostList(pidList: string[]) {
  const startTime = Date.now();
  const chunkSize = 20; // 每次处理的帖子数量
  const delay = 3000; // 每次请求之间的延迟时间（毫秒）
